...
impl PatternDetector {
    pub fn new() -> Self {
        let patterns = vec![
            ...
            CompiledPattern {
                name: "port",
                regex: Regex::new(r"(?i)(?:listening on|server started on|port:?)\s*(?::)?(\d{1,5})").unwrap(),
                extractor: Box::new(|caps| {
                    caps.get(1)
            ...
            CompiledPattern {
                name: "error",
                regex: Regex::new(r"(?i)(?:error|exception|failed):\s*(.+)").unwrap(),
                extractor: Box::new(|caps| {
                    caps.get(1).map(|m| DetectedPattern::Error(m.as_str().to_string()))
                }),
            },
            CompiledPattern {
                name: "file_path",
