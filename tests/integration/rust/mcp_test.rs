...
async fn setup_test_server() -> (McpServer, TempDir) {
    ...
    let process_manager = Arc::new(ProcessManager::new(log_storage.clone(), log_tx).unwrap());
    
    let config = agent_process_manager::config::Config::default();
    let server = McpServer::new(process_manager, log_storage, config, None).await.unwrap();
